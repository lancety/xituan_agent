...
| **待完成** | ① 部署 SIH 样本时：**手动更新 Lambda 环境变量 `SIZE_OPTS`**；② Gate 后 Phase N：去掉 SIH `jpeg/jpg` **并清理展示层 jpg→SIH 过渡** |
| **Gate** | C4 全量预生成完成：业务 DB/展示主 path 仅为 `.webp`/`.png`；SIH 可去掉 `jpeg/jpg`；客户端不再拼 `format=jpeg`；**`_w*` thumb 不再需要 jpg→SIH 回退** |
| **创建日** | 2026-09-05 |
| **定案补记** | 2026-09-06：**终态只保留 webp + png**。过渡期 jpg 走 SIH / `_w` 不可用时 SIH 回退；回填完成后**不应继续支持** jpeg/jpg 作为业务图主格式，**过渡分支必须删掉**。 |
...
| **N（format + 展示过渡清理，Gate 后）** | SIH 样本 **只保留 `webp\|png`**；去掉 jpeg/jpg；**删除** codebase/客户端「jpg 无 `_w*` → 仍走 SIH」过渡逻辑与注释 | pending | **终态：不再支持 jpeg/jpg；渐进加载仅 `_w` + SIH(512\|1024)\|canonical** |
...
- [ ] Phase N：**清理展示过渡** — `preferStoredVariantPath` / `getProgressivePair` 及相关调用处不再保留「jpg 过渡仍 SIH」分支与文案；业务图一律 webp/png + `_w*`
...
- [ ] 若收紧：同步部署 dev/staging/prod SIH；回归确认无业务主 path 仍为 `.jpg`/`.jpeg`
- [ ] **删除 jpg→SIH 展示过渡**：`site-image-display.util` / `content.util` 注释与分支、Site/CMS/微信展示文案中的「过渡期 jpg 走 SIH / `_w64` jpg fallback」一律清理
- [ ] 环境变量 `SIZE_OPTS` 是否去掉临时 `2048`（若曾追加）
- [ ] 更新 `devGuide/sih-image-size-phase-a.md`、`async-lambda-jobs-framework.md`
